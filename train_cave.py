...
from dataset import CustomDataset_Cave
import torch

def adjust_learning_rate(optimizer, epoch, init_lr, decay_epochs=[250, 300, 350],
                         decay_factors=[1, 0.5, 0.1]):
    lr = init_lr
...
def train(opt):
    ...
    net.train()
    train_dataset = CustomDataset_Cave(path=opt.train_path)
...
def test(opt):
    criterion_mse = nn.MSELoss().to(device)
    net.eval()
    test_dataset = CustomDataset_Cave(path=opt.test_path)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--train_epoch", type=int, default=350)
    parser.add_argument('--train_size', type=int, default=24)
    parser.add_argument('--test_size', type=int, default=7)
    parser.add_argument('--batchSize', type=int, default=8)
    parser.add_argument('--testBatchSize', type=int, default=1)
    parser.add_argument("--lambda", type=float, default=1024, dest="lmbda", help="Lambda for rate-distortion tradeoff.")#lamda in our paper
    ...
    parser.add_argument("--train_path", type=str, default=" Cave txt Path to train")
    parser.add_argument("--test_path", type=str, default="Cave txt Path to test")
